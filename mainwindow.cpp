...
Binaural::CCore Cores[nbInstruMax];	// Core interface
bool sound3DActive = false;
...
struct pa_Data
{
     ...
     shared_ptr<Binaural::CSingleSourceDSP>	A3DSource;
     Common::CTransform *sourcePosition = nullptr;
     Common::CEarPair<CMonoBuffer<float>> buffer3DProcessed;
     Common::CEarPair<CMonoBuffer<float>> buffer3DReverb;
     CMonoBuffer<float> buffer3D;
     bool handle3DMix = false;
};
...
class offLineMixer : public QThread
{
public:
    pa_Data *data;
    quint64 position;
...
offLineMixer *offLineMixers[nbInstruMax];
...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    ERRORHANDLER3DTI.SetVerbosityMode(VERBOSITY_MODE_SILENT);
    Common::TAudioStateStruct audioState;											// Audio State struct declaration
    audioState.bufferSize = FRAME_SIZE;											// Setting buffer size and sample rate
    audioState.sampleRate = SAMPLE_RATE;

    for (int index=0; index<nbInstruMax; index++) {
    Cores[index].SetAudioState(audioState);												// Applying configuration to core
    Cores[index].SetHRTFResamplingStep(15);
    pa_data[index].listener = Cores[index].CreateListener();												// First step is creating listener

    pa_data[index].A3DSource = Cores[index].CreateSingleSourceDSP();							 // Creating audio source
    pa_data[index].sourcePosition = new Common::CTransform();
    pa_data[index].sourcePosition->SetPosition(Common::CVector3(0, 0, 0));						 // Setting source in x=0,y=2,z=0 (on the left)
    pa_data[index].A3DSource->SetSourceTransform(*pa_data[index].sourcePosition);
    pa_data[index].A3DSource->SetSpatializationMode(Binaural::TSpatializationMode::HighQuality);	 // HighPerformance vs HighQuality // Choosing high quality mode for anechoic processing
    pa_data[index].A3DSource->DisableNearFieldEffect();												 // Audio source will not be close to listener, so we don't need near field effect
    pa_data[index].A3DSource->EnableAnechoicProcess();												 // Setting anechoic and reverb processing for this source
    pa_data[index].A3DSource->EnableDistanceAttenuationAnechoic();
    pa_data[index].A3DSource->EnableDistanceAttenuationReverb();

    Common::CTransform listenerPosition = Common::CTransform();						// Setting listener in (0,0,0)
    listenerPosition.SetPosition(Common::CVector3(0, 0, 0));
    pa_data[index].listener->SetListenerTransform(listenerPosition);
    pa_data[index].listener->DisableCustomizedITD();												// Disabling custom head radius
    if ((QFile::exists("HRTF/3DTI/3DTI_HRTF_IRC1031_512s_44100Hz.3dti-hrtf")) && QFile::exists("BRIR/SOFA/3DTI_BRIR_medium_48000Hz.sofa")) sound3DActive = true;
    HRTF::CreateFrom3dti("HRTF/3DTI/3DTI_HRTF_IRC1031_512s_44100Hz.3dti-hrtf", pa_data[index].listener);
    pa_data[index].environment = Cores[index].CreateEnvironment();											// Creating environment to have reverberated sound
    pa_data[index].environment->SetReverberationOrder(TReverberationOrder::BIDIMENSIONAL);				// Setting number of ambisonic channels to use in reverberation processing
    BRIR::CreateFromSofa("BRIR/SOFA/3DTI_BRIR_medium_48000Hz.sofa", pa_data[index].environment);										// Loading SOFAcoustics BRIR file and applying it to the environment
    }
    ...
    connect(ui->button3DRoomReverb, SIGNAL(clicked()), this, SLOT(chooseRoomReverb()));
    connect(ui->buttonHTRF, SIGNAL(clicked()), this, SLOT(chooseHTRF()));
...
void MainWindow::newDevice()
{
    ...
    nbInstru ++;
    int x = 0;
    offLineMixers[index] = new offLineMixer(&pa_data[index]);
    ...
    ui->gridLayout->addWidget(Micro[index], index*2, x+2, 1, 4);
#else
    ui->gridLayout->addWidget(Ecouteur[index], index*2, x, 1, 7);
#endif
    ...
    xPos[index]->setMinimum(-10);
    xPos[index]->setMaximum(10);
    xPos[index]->setDecimals(1);
    xPos[index]->setSingleStep(0.1);
    //xPos[index]->setSuffix(" dB");
    //xPos[index]->setStyleSheet("QSpinBox { background-color:#ffd866 }");
    xPos[index]->setPrefix("X : ");
    xPos[index]->setFocusPolicy(Qt::ClickFocus);
    ui->gridLayout->addWidget(xPos[index], index*2+1, x++, 1, 1);
    xPos[index]->setEnabled(sound3DActive);
    connect(xPos[index], SIGNAL(valueChanged(double)), this, SLOT(update3D()));

    yPos[index] = new QDoubleSpinBox(ui->groupBox);
    yPos[index]->setMinimum(-10);
    yPos[index]->setMaximum(10);
    yPos[index]->setDecimals(1);
    yPos[index]->setSingleStep(0.1);
    //yPos[index]->setSuffix(" dB");
    //yPos[index]->setStyleSheet("QSpinBox { background-color:#ffd866 }");
    yPos[index]->setPrefix("Y : ");
    yPos[index]->setFocusPolicy(Qt::ClickFocus);
    ui->gridLayout->addWidget(yPos[index], index*2+1, x++, 1, 1);
    yPos[index]->setEnabled(sound3DActive);
    connect(yPos[index], SIGNAL(valueChanged(double)), this, SLOT(update3D()));
    ...
    // transfered when load config is finished connect(L_In[index], SIGNAL(valueChanged(int)), this, SLOT(updateLevel()));
...
void MainWindow::loadConfig()
{
    ...
    for (int n=0; n<nbInstru; n++) {
        ...
        if (ok) xPos[n]->setValue(V);

        str = settings.value(QString("/3DYPos/%1").arg(n+1)).toString();
        V = str.toDouble(&ok);
        if (ok) yPos[n]->setValue(V);
    ...
    for (int n=0; n<nbInstru; n++) { connect(L_In[n], SIGNAL(valueChanged(int)), this, SLOT(updateLevel())); }
    updateLevel();
...
void MainWindow::saveConfig()
{
    ...
    for (int n=0; n<nbInstru; n++) {
        ...
        settings.setValue(QString("/sound3D/%1").arg(n+1), pa_data[n].sound3D);
        settings.setValue(QString("/reverb3D/%1").arg(n+1), pa_data[n].reverb3D);
        settings.setValue(QString("/3DXPos/%1").arg(n+1), xPos[n]->value());
        settings.setValue(QString("/3DYPos/%1").arg(n+1), yPos[n]->value());
        for (int i=0; i<nbInstru; i++) {
...
int MainWindow::Callback(const void *input,
             ...
             void *userData)
 {
    ...
    // add the Mix of all other tracks
        for (int n = 0; n < nbInstru; n++) {
            if (n != data->myIndex) {
                if (!mix_track_Hor_Vert[n][data->myIndex].mix_send_list.isEmpty()) {
                L += mix_track_Hor_Vert[n][data->myIndex].mix_send_list.first()->frame[i].L;
    ...
    // Here we delete used mixed data
    for (int n = 0; n < nbInstru; n++) {
        if (n != data->myIndex) {
            if (!mix_track_Hor_Vert[n][data->myIndex].mix_send_list.isEmpty()) {
    mix_track_Hor_Vert[n][data->myIndex].mix_send_list.takeFirst(); } } }
    ...
    // Purge mix data in case of miss synchronisation, remove one frames of each track if too much mixed frames
    for (int n = 0; n < nbInstru; n++) {
...
void MainWindow::ON()
{
    ...
    for (int n=0; n<nbInstru; n++) {
        ...
        if ((Ecouteur[n]->currentIndex() != -1) && (firstDevice)) {
            pa_data[n].handle3DMix = true;
            ui->log->append(Nom[n]->text() + " handle 3D mix");
            firstDevice = false;
        }
    }
    ...
    ui->button3DRoomReverb->setEnabled(false);
    ui->buttonHTRF->setEnabled(false);
...
void MainWindow::OFF()
{
    ...
    ui->buttonRecord->setEnabled(false);
    ui->button3DRoomReverb->setEnabled(true);
    ui->buttonHTRF->setEnabled(true);
...
void MainWindow::chooseHTRF()
{
    QString name = QFileDialog::getOpenFileName(this, tr("Open files"), "", tr("Room Reverb Files (*.3dti-hrtf)"));
    if (!name.isEmpty()) {
...
void MainWindow::chooseRoomReverb()
{
...
void MainWindow::sound3DChanged(QCheckBox* checkbox)
{
    if (QGuiApplication::keyboardModifiers().testFlag(Qt::ShiftModifier)) {
        Qt::CheckState state = Qt::Unchecked;
        for (int n=0; n<nbInstru; n++) {
            if (sound3D[n] == checkbox) { state = sound3D[n]->checkState(); } }
        for (int n=0; n<nbInstru; n++) { sound3D[n]->setCheckState(state); } }

    for (int n=0; n<nbInstru; n++) {
        if (sound3D[n]->isChecked()) { pa_data[n].sound3D = true; reverb3D[n]->setEnabled(sound3DActive); }
        else { pa_data[n].sound3D = false; reverb3D[n]->setEnabled(false); } }
}


void MainWindow::reverb3DChanged(QCheckBox* checkbox)
{
    if (QGuiApplication::keyboardModifiers().testFlag(Qt::ShiftModifier)) {
        Qt::CheckState state = Qt::Unchecked;
        for (int n=0; n<nbInstru; n++) {
            if (reverb3D[n] == checkbox) { state = reverb3D[n]->checkState(); } }
        for (int n=0; n<nbInstru; n++) { reverb3D[n]->setCheckState(state); } }

    for (int n=0; n<nbInstru; n++) {
        if (reverb3D[n]->isChecked()) pa_data[n].reverb3D = true;
        else pa_data[n].reverb3D =false; }
}
...
int64_t MainWindow::loadMonoWavFile(QString fileName, float *&wavSound, bool &converted)
{
    converted = true;
    int64_t wavSize = 0;
    SF_INFO sfInfo;
    ...
    if (sfInfo.channels != 1) {
        QMessageBox msgBox;
        msgBox.setText("File " + fileName + " is not mono\nCan't load file");
    ...
    else if ((sfInfo.samplerate == 44100) && (SAMPLE_RATE == 48000))
    {
        ...
        for (uint32_t idx = 0; idx < dstSize; idx++) {
               ...
               wavSound[idx] = ((1.0f - coef) * wav[p1] + coef * wav[p2]);
        }
...
int64_t MainWindow::loadWavFile(QString fileName, float *&wavSound, bool &converted)
{
    ...
    if (sfInfo.channels != 2) {
        QMessageBox msgBox;
        msgBox.setText("File " + fileName + " is not stereo\nCan't load file");
        msgBox.setStandardButtons(QMessageBox::Abort);
        msgBox.exec();
        wavSound = nullptr;
        return 0;
...
void MainWindow::setFile(QString str)
{
    ...
    for (int n=0; n<nbInstru; n++) {
        ...
        pa_data[n].wavRecord = nullptr;
            ...
            {
                bool converted = false;
                quint64 wavSize = loadMonoWavFile(wavFileName, pa_data[n].wavRecord, converted);
                if (wavSize >= Frames) {
        ...
        if (pa_data[n].wavRecord == nullptr) { pa_data[n].wavRecord = new float[Frames];
            if (!pa_data[n].wavRecord) {
                ...
                msgBox.exec();
                return; }
        for (quint64 f=0; f<(Frames); f++) { pa_data[n].wavRecord[f] = 0; } }
...
void MainWindow::saveFiles(int track)
{
    ...
    for (int n=b; n<e; n++) {
        ...
        sf_write_float(outfile, pa_data[n].wavRecord, pa_data[n].frames) ;
        sf_write_sync(outfile);
        sf_close(outfile);
                ...
                pa_data[i].mode = pa_data[n].mode;
                    switch (pa_data[n].mode) {
                        case Playback : ModeButton[i]->setIcon(QIcon(":/images/replay"));
                                        ...
                                        L_In[i]->setValue(pa_data[i].PlayVol);
                                        break;
                        case Record :   ModeButton[i]->setIcon(QIcon(":/images/microrouge"));
                                        ...
                                        L_In[i]->setValue(pa_data[i].RecVol);
                                        break;
                        case Listen :   ModeButton[i]->setIcon(QIcon(":/images/microvert"));
                                        ...
                                        L_In[i]->setValue(pa_data[i].ListenVol);
                                        break;
                        case trackOff :
...
void MainWindow::update3D()
{
    for (int n=0; n<nbInstru; n++) {
        pa_data[n].sourcePosition->SetPosition(Common::CVector3(xPos[n]->value(), yPos[n]->value(), 0));
        pa_data[n].A3DSource->SetSourceTransform(*pa_data[n].sourcePosition);
